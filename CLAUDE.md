...
| Muovere la vista 3D (girarla, avvicinarla, scegliere un pianeta) | `solInizializzaGesti()` (sezione 7.7): un dito gira (`sol.az`, `sol.elev`), due pizzicano, la rotella avvicina, un tocco secco sceglie il pianeta più vicino con `solTocco()`. I tasti dei punti di vista non saltano: muovono `sol.elevVoluta` e `sol.zoomVoluto`, e ci si arriva scivolando dentro `solCiclo()` |
