...
|---|---|---|
| `index.html` | ~1.665 | Struttura statica: testata, 6 viste, 10 modali. Nessuna logica. |
...
| 8205–8500 | **7.1-quater** Il punto scelto sul mappamondo: la mappa Leaflet per i luoghi che un nome non ce l'hanno, con le coordinate a mano sotto | `luogoMappa` (**8237**), `luogoMappaCostruisci()` (**8283**), `luogoMappaScegli()` (**8334**), `luogoMappaAggiornaLettura()` (**8372**), `apriMappaLuogoCielo()` (**8420**), `luogoMappaUsa()` (**8447**), `inizializzaMappaLuogo()` (**8459**) |
...
| Scegliere un punto che non ha un nome (un passo di montagna, una spiaggia, il mezzo dell'oceano) | sezione **7.1-quater**: `apriMappaLuogoCielo()` apre `modale-luogo-cielo`, un tocco sulla mappa chiama `luogoMappaScegli()` e il tasto «Guarda il cielo da qui» passa la palla a `skyUsaLuogoVista()`, cioè alla stessa strada di una città scelta in elenco. Le coordinate a mano stanno lì dentro (`#luogo-lat`/`#luogo-lon`), che è l'unico posto in cui hanno spazio. Il nome del punto lo mette `nomeLuogoVicino()` se un paese c'è entro `LUOGO_NOME_KM`; se non c'è, la lettura dice comunque qual è il posto più vicino e a che distanza (`luogoCittaPiuVicina()`). Stili `.luogo-*` e `#mappa-luogo` in `style.css` |
