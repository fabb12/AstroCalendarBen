...
| `sw.js` | ~105 | Service worker. `CACHE_NAME` va incrementato a ogni rilascio (oggi `astrocal-v46`). |
...
- **Dopo ogni modifica ai file dell'app, incrementa `CACHE_NAME` in `sw.js`**
  (oggi `astrocal-v46`): senza questo, chi ha già installato la PWA continua a
