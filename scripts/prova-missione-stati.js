...
const ctx=vm.createContext({console,Date:Clock,Astronomy,window:{},document:doc,
  ...
  osservatoreCorrente:()=>obs, sky:{observer:obs,sensori:false,assoluto:false,seguiTelefono:false},
...
assert.equal(run('miss.attiva.tappe[0].esito'),null);assert.equal(run('miss.attiva.tappe[0].aiuto'),1);
...
for(const lang of ['it','en']){ctx.astroI18n.lingua=lang;
 for(const mode of ['sfida','imparare','bambini','stupore'])for(const type of ['luna','pianeta','stella','costellazione','profondo'])for(let v=0;v<3;v++) {
...
console.log('PASS: actual ephemerides, anonymous UI, manual completion blocked, wrong/correct selections, live time/location/terrain checks, discovery persistence, escaped notes, 120 content combinations, sensors, cleanup');
